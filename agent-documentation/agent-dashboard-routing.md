...
Cross-check [[common-spec|Common Dashboard Spec]], [[common-spec|Common Dashboard Spec]], and [[common-spec|Common Dashboard Spec]].


This file defines where each agent writes dashboard output.

Use it with [[agent-output-contracts|Agent Output Contracts]], [[agent-workflows|Agent Workflows]], and [[common-spec|Common Dashboard Spec]].
...
| Linda | Primary for project status, insights, must-look findings, and one-day work | Secondary when cleanup requires structural approval | Primary when ambiguity, overlap, missing source state, or sequencing blocks trust | No | Secondary when findings imply a decision record | No | No |
